...
export async function getEquipmentsForProcessGroup(
  ...
  const [rows] = await conn.execute<any[]>(
    `SELECT
       ...
       COALESCE(em.default_temperature, h.default_temperature)   AS default_temperature,
       COALESCE(em.default_pressure,    h.default_pressure)      AS default_pressure,
       COALESCE(em.batch_operation_time, h.batch_operation_time) AS batch_operation_time,
       COALESCE(em.default_time,         h.default_time)         AS default_time
...
export async function getProcessGroupsForProduct(args: {
  ...
  return [
    ...
    ...(manualRows as any[]).map((r: any) => toInfo(r, "MANUAL")),
  ];
}
...
async function createDefaultRowsForGroup(
  ...
  for (let idx = 0; idx < equipments.length; idx++) {
    ...
    const durationMin  =
      cycleTotalMin != null
        ? cycleTotalMin + (heatingMin - eqDefaultHeat)   // 총소요시간 = 사이클 + (공정가열 - 설비기본가열)
        : heatingMin;                                      // 사이클 미정의 시 공정가열시간으로 폴백
