...
export async function createSingleBatch(
  ...
  const batchId = await createBatch({
    ...
    // auto 모드: 즉시 completed (백오피스 사후 입력)
    // manual 모드: planned (수동 시작→완료)
    status: input.mode === "auto" ? "completed" : "planned",
  });
  ...
  // === 2.5. BOM batch_target_kg 사전 조회 (CCP + form_records 양쪽에서 사용) ===
  ...
  // bomBatchKg는 위(2.5단계)에서 이미 조회 완료
