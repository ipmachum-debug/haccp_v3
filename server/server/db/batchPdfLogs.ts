...
export async function getFailedTasks(tenantId?: number): Promise<RetryTaskRecord[]> {
  ...
  return await db
    ...
    .where(and(eq(hBatchCompletionRetries.tenantId, tenantId), eq(hBatchCompletionRetries.status, "failed")))    .orderBy(desc(hBatchCompletionRetries.createdAt));
}
...
export async function retryFailedTask(taskId: number, tenantId?: number): Promise<{ success: boolean; message: string }> {
  ...
  const task = await db
    .select()
    .from(hBatchCompletionRetries)
    .where(and(eq(hBatchCompletionRetries.tenantId, tenantId), eq(hBatchCompletionRetries.id, taskId)))    .limit(1);
