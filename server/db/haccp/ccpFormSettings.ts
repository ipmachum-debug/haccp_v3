...
export async function syncCcpRowsToFormRows(params: {
  ...
  for (const fr of formRecords as any[]) {
    ...
    // CCP-4P: 일일 통합 기록지이므로 당일 전체 배치/SKU 수가 일치할 때만 skip
    // ★ 일괄 배치 생성 시: 첫 배치 sync → 혼자 있을 때 22행 생성 → 두번째 배치에서 skip이 되면
    //    나머지 배치의 SKU가 누락됨. 따라서 "당일 실제 SKU 개수"와 "기존 행 개수"가 일치할 때만 skip
    if (ccpType === "CCP-4P" && existingSeqs.size > 0 && fr.work_date) {
      // 연결된 배치가 삭제되었는지 먼저 확인
      ...
      // ★ 정확한 skip 판정: 기존 form_rows 에 당일 전체 제품 이름이 모두 포함되어 있는지 비교
      //    기존 방식(count 비교)은 sensitivity row 수 편차로 오판 발생 (특히 3번째 배치 때)
      //    → product_name set 기반으로 정확히 판정
      let existingProductNames = new Set<string>();
      try {
        const [existingProdRows] = await rawConn.execute<any[]>(
          `SELECT DISTINCT product_name FROM h_ccp_form_rows
           WHERE form_record_id = ? AND tenant_id = ?
             AND product_name IS NOT NULL AND product_name <> ''`,
          [formRecordId, tenantId],
        );
        for (const r of (existingProdRows as any[])) {
          if (r.product_name) existingProductNames.add(String(r.product_name));
        }
      } catch { /* 실패 시 빈 set 유지 → 강제 재생성 */ }

      let expectedProductNames = new Set<string>();
      try {
        const [expectedProdRows] = await rawConn.execute<any[]>(
          `SELECT DISTINCT COALESCE(p.product_name, fr2.product_name, '') AS product_name
           FROM h_batches b
           LEFT JOIN h_products_v2 p ON p.id = b.product_id AND p.tenant_id = b.tenant_id
          ...
          [tenantId, fr.work_date],
        );
        for (const r of (expectedProdRows as any[])) {
          const nm = String(r.product_name || "").trim();
          if (nm) expectedProductNames.add(nm);
        }
      } catch { /* 실패 시 빈 set 유지 → 강제 재생성 */ }

      // 기존 product set ⊇ expected product set 일 때만 skip
      // (빠진 제품이 있으면 재생성)
      const allProductsCovered =
        expectedProductNames.size > 0 &&
        Array.from(expectedProductNames).every((nm) => existingProductNames.has(nm));

      if (linkedBatchExists && allProductsCovered) {
        console.log(
          `[syncCcpRowsToFormRows] CCP-4P skip: formRecord=${formRecordId}, ` +
            `existing=[${Array.from(existingProductNames).join(",")}], ` +
            `expected=[${Array.from(expectedProductNames).join(",")}]`,
        );
        continue; // 이미 당일 모든 제품이 반영됨
      }
      console.log(
        `[syncCcpRowsToFormRows] CCP-4P 재생성: formRecord=${formRecordId}, ` +
          `기존=[${Array.from(existingProductNames).join(",")}], ` +
          `필요=[${Array.from(expectedProductNames).join(",")}]`,
      );
      // 그 외: 기존 행 삭제 후 전체 재생성 (새 배치 반영)
      await rawConn.execute(
        `DELETE FROM h_ccp_form_rows WHERE form_record_id = ? AND tenant_id = ?`,
        [formRecordId, tenantId],
      );
