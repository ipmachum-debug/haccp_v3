...
export async function generateAIBriefing(tenantId: number, userName: string): Promise<AIBriefingResult> {
  ...
  try {
    ...
    try {
        ...
        `SELECT m.material_name, m.safety_stock_level,
         ...
         LEFT JOIN (
           SELECT bi.material_id,
                  SUM(COALESCE(bi.actual_quantity, bi.planned_quantity)) / GREATEST(DATEDIFF(NOW(), MIN(b.start_time)), 1) AS daily_avg
      ...
      for (const row of (lowStockRows as any[])) {
        const stock = parseFloat(row.current_stock || "0");
        const dailyAvg = parseFloat(row.daily_avg || "0");
        ...
        items.push({
          ...
          icon: '🚨',
          label: '재고',
          message: `${row.material_name} 부족${daysText}`,
          severity: daysLeft !== null && daysLeft <= 2 ? 'critical' : 'warning',
          actionLabel: '발주 확인',
    ...
    try {
      const [costRows]: any = await pool.execute(
        `SELECT m.material_name,
                ...
                ROUND((curr_price.avg_price - prev_price.avg_price) / GREATEST(prev_price.avg_price, 1) * 100, 0) AS change_pct
         FROM h_materials m
         JOIN (
         ...
         WHERE m.tenant_id = ?
           AND ABS(curr_price.avg_price - prev_price.avg_price) / GREATEST(prev_price.avg_price, 1) > 0.05
         ORDER BY ABS(curr_price.avg_price - prev_price.avg_price) / GREATEST(prev_price.avg_price, 1) DESC
         LIMIT 2`,
        [tenantId, tenantId, tenantId]
      );

      if ((costRows as any[]).length > 0) {
        const names = (costRows as any[]).map((r: any) => r.material_name).join(', ');
        ...
        items.push({
          ...
          icon: '💰',
          label: '원가',
          message: `${names} 단가 ${direction} 중 (최대 ${maxPct > 0 ? '+' : ''}${maxPct}%)`,
          severity: maxPct >= 10 ? 'critical' : 'warning',
          actionLabel: '원가 확인',
          actionUrl: '/accounting/material-ledger',
        });
      }
    } catch (e) { console.error('[briefing] 원가 변화 조회 실패:', e); }

    // ══════════════════════════════════════════════
    // 3. ⚠️ 품질/HACCP 이상 - CCP 미기록, 온도 이탈
    // ══════════════════════════════════════════════
    try {
      // CCP 미기록 또는 이탈
      const [ccpRows]: any = await pool.execute(
        `SELECT
           SUM(CASE WHEN status = 'deviation' THEN 1 ELSE 0 END) AS deviations,
           SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) AS pending
         FROM h_ccp_records
         WHERE tenant_id = ? AND DATE(recorded_at) >= DATE_SUB(?, INTERVAL 1 DAY)`,
        [tenantId, today]
      );

      const deviations = Number((ccpRows as any[])?.[0]?.deviations || 0);
      const pending = Number((ccpRows as any[])?.[0]?.pending || 0);
      ...
      if (deviations > 0) {
        items.push({
          ...
          icon: '⚠️',
          label: '품질',
          message: `CCP 기준 이탈 ${deviations}건 발생`,
          severity: 'critical',
          actionLabel: 'CCP 확인',
          actionUrl: '/ccp-management',
        });
      } else if (pending > 0) {
        items.push({
          type: 'quality_issue',
          icon: '⚠️',
          label: '품질',
          message: `CCP 기록 미완료 ${pending}건`,
          severity: 'warning',
          actionLabel: 'CCP 확인',
          actionUrl: '/ccp-management',
        });
      }
    } catch (e) { console.error('[briefing] CCP 조회 실패:', e); }

    // ══════════════════════════════════════════════
    // 4. 📉 생산 이상 - 오늘 생산량 vs 평균
    // ══════════════════════════════════════════════
    try {
      const [prodRows]: any = await pool.execute(
        `SELECT
         ...
         LEFT JOIN (
           SELECT COUNT(*) AS cnt, SUM(planned_quantity) AS total_qty
           FROM h_batches WHERE tenant_id = ? AND DATE(COALESCE(start_time, created_at)) = ?
         ) today_prod ON 1=1
         LEFT JOIN (
           SELECT COUNT(*) / GREATEST(COUNT(DISTINCT DATE(COALESCE(start_time, created_at))), 1) AS avg_daily_batches,
                  SUM(planned_quantity) / GREATEST(COUNT(DISTINCT DATE(COALESCE(start_time, created_at))), 1) AS avg_daily_qty
           FROM h_batches WHERE tenant_id = ?
             AND COALESCE(start_time, created_at) >= DATE_SUB(NOW(), INTERVAL 30 DAY)
    ...
    // ══════════════════════════════════════════════
    try {
      // 재고 위험에서 이미 잡힌 것 제외, 순수 발주 필요 건만
      if (!items.some(i => i.type === 'inventory_risk')) {
        ...
        if ((orderRows as any[]).length > 0) {
          const names = (orderRows as any[]).map((r: any) => r.material_name).join(', ');
          items.push({
            type: 'order_needed',
            icon: '📦',
            label: '발주',
            message: `${names} 발주 필요 (3일 내 소진 예상)`,
            severity: 'warning',
            actionLabel: '발주 확인',
            actionUrl: '/inventory-management',
          });
  ...
  const greeting = topItems.length > 0
    ? GREETINGS_WITH_ITEMS[Math.floor(Math.random() * GREETINGS_WITH_ITEMS.length)]
    : GREETINGS_CLEAR[Math.floor(Math.random() * GREETINGS_CLEAR.length)];
