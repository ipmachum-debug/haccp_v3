...
export async function addCcpRow(data: {
  ...
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const [row] = await db.insert(hCcpRows).values({
 ...
 * CCP 점검 기록 조회
 */
export async function getCcpRows(instanceId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db
    .select()
    .from(hCcpRows)
    .where(eq(hCcpRows.instanceId, instanceId))
...
export async function approveCcpInstance(
  ...
  await db
    ...
    .where(eq(hCcpInstances.id, instanceId));
}
