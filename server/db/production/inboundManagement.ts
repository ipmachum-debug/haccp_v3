...
export async function createInboundReceipt(params: {
  ...
  const [material] = await db
    ...
    .from(hMaterials)
    .where(and(
      ...(tenantId ? [eq(hMaterials.tenantId, tenantId as number)] : []),
  ...
  const result = await withTransaction(async (conn) => {
    ...
    // PR-§5.2-2: material_id 직접 작성 (params.materialId — 입고 시점에 명시적으로 받음)
    await conn.execute(
...
export async function getInboundHistory(params: {
  ...
  // 필터 조건 구성
  // ★ 2026-04-13 수정: h_materials 는 LEFT JOIN 으로 변경 →
  //    material_id NULL 인 LOT (과거 purchase 자동 생성 데이터) 도 표시
  //    tenant 격리는 h_inventory_lots 자체 tenant_id 로 이동
  const conditions: any[] = [eq(hInventoryTransactions.transactionType, "receipt")];
  if (tenantId) {
    conditions.push(eq(hInventoryLots.tenantId, tenantId as number));
  }
  ...
  if (params.startDate) {
    conditions.push(gte(hInventoryLots.receiptDate, formatLocalDate(params.startDate) as any));
  }

  if (params.endDate) {
    conditions.push(lte(hInventoryLots.receiptDate, formatLocalDate(params.endDate) as any));
  }
