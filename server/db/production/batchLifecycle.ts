...
/**
 * PR-W3 (2026-04-26): 배치 자동출고/완료 시 transaction_date 결정 헬퍼
 *
 * 우선순위: completed_at → planned_date → 오늘 KST (폴백)
 * 반환: YYYY-MM-DD (KST)
 *
 * 배경: 기존에는 NOW() 시점만 기록 → 4/9 배치를 4/26 에 처리하면 4/26 으로
 * 잘못 기록되어 일별 그래프 왜곡. 이 함수로 일관되게 배치 일자 기준으로 정정.
 */
function resolveBatchTransactionDate(batch: { completedAt?: any; plannedDate?: any; completed_at?: any; planned_date?: any }): string {
  const candidate = batch?.completedAt ?? batch?.completed_at ?? batch?.plannedDate ?? batch?.planned_date ?? null;
...
export async function completeBatch(params: {
  ...
  try {
    ...
    for (const { input, materialName } of batchInputs) {
      ...
      // ★ PR-W3 (2026-04-26): transaction_date 부정합 수정
      //   기존 NOW() 방식 → batch.completed_at → planned_date → 오늘 우선순위
      //   source_type → 'BATCH' 로 표준화 (신규 표기. autoMaterialIssue.ts 와 일치)
      try {
        if (pool) {
      ...
      // 수불부 반영
      // ★ PR-W3: ledger_date 도 배치 일자 기준 (transaction_date 와 일치)
      try {
        if (pool) {
          const txnDate = resolveBatchTransactionDate(existingBatch);
          await pool.execute(
  ...
  try {
    // 6-1. 문서 자동 생성 (★ 2026-04-15: tenantId 전달 - 테넌트 격리)
  ...
  // ★ 7. 배치 완료 통합 훅 (PR #274) — 3개 완료 경로 공통 후처리
  //   - actual_quantity NULL 보강 (production_sku_output 합계 우선)
  //   - h_batch_inputs 0건 경고
  //   - h_daily_reports production_daily 캐시 무효화
  let hookResult: any = null;
  try {
    const { runBatchCompletionHooks } = await import(
      "../../lib/production/batchCompletionHooks"
    );
    hookResult = await runBatchCompletionHooks(batchId, existingBatch.tenantId, {
      source: "completeBatch",
    });
    if (hookResult.warnings.length > 0) {
      console.warn(
        `[Batch Completion] 통합 훅 경고 (배치#${batchId}):`,
        hookResult.warnings,
      );
    }
    if (hookResult.errors.length > 0) {
      console.error(
        `[Batch Completion] 통합 훅 오류 (배치#${batchId}):`,
        hookResult.errors,
      );
    }
  } catch (hookErr) {
    console.error(`[Batch Completion] 통합 훅 실행 실패:`, hookErr);
  }
  ...
  return {
    ...
    data: {
      ...
      completionHooks: hookResult ? {
        backfilledActualQuantity: hookResult.backfilledActualQuantity,
        batchInputsCount: hookResult.batchInputsCount,
        invalidatedCacheRows: hookResult.invalidatedCacheRows,
