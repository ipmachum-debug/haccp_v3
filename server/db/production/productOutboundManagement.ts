...
export async function createProductLotFromBatch(params: {
  ...
  // 입고 트랜잭션 기록
  // PR-§5.2-2 노트: 본 INSERT 는 *제품 LOT 입고* 트랜잭션이므로 material_id 는 NULL.
...
export async function ensureBatchLots(batchId: number, tenantId: number): Promise<{
  ...
  if ((skuRows as any[]).length > 0) {
    ...
    for (const sku of skuRows as any[]) {
      ...
      // 입고 트랜잭션 기록
      // PR-§5.2-2 노트: 본 INSERT 는 *제품 LOT 보강* 트랜잭션이므로 material_id 는 NULL.
      await db.insert(hInventoryTransactions).values({
        tenantId,
        lotId,
        transactionType: "inbound",
  ...
  // PR-§5.2-2 노트: 본 INSERT 는 *제품 LOT 보강 (fallback)* 트랜잭션이므로 material_id 는 NULL.
  await db.insert(hInventoryTransactions).values({
    tenantId,
    lotId,
    transactionType: "inbound",
...
export async function createProductOutbound(params: {
  ...
  if (params.lotId) {
    ...
    // 출고 트랜잭션 기록
    // PR-§5.2-2 노트: 본 INSERT 는 *제품 LOT 출고* 트랜잭션이므로 material_id 는 NULL.
    if (db) {
      await db.insert(hInventoryTransactions).values({
        tenantId,
        lotId: params.lotId,
