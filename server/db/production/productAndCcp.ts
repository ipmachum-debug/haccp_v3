...
export async function getAllProducts(tenantId?: number) {
  ...
  if (tenantId) conditions.push(eq(hProductsV2.tenantId, tenantId as number));
...
export async function getAllCcpTemplates(tenantId?: number) {
  ...
  if (tenantId) {
    return await db
      .select()
      .from(hCcpTemplates)
      .where(eq(hCcpTemplates.tenantId, tenantId as number))
...
export async function getCcpTemplateById(id: number, tenantId?: number) {
  ...
  if (tenantId) conditions.push(eq(hCcpTemplates.tenantId, tenantId as number));
...
export async function getCcpInstancesByBatchId(batchId: number, tenantId?: number) {
  ...
  const instanceIds = (instances as unknown as Record<string, unknown>[]).map((r) => r.id as number);
  ...
  return (instances as unknown as Record<string, unknown>[]).map((inst) => ({
    ...inst,
 ...
 * CCP 인스턴스의 점검 행 조회
 */
export async function getCcpRowsByInstanceId(instanceId: number, tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const { hCcpRows } = await import("../../../drizzle/schema.js");
  const { eq } = await import("drizzle-orm");
 ...
 * CCP 인스턴스 상태 업데이트
 */
export async function updateCcpInstanceStatus(instanceId: number, status: "draft" | "submitted" | "approved" | "rejected", userId?: number, tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const { hCcpInstances } = await import("../../../drizzle/schema.js");
  const { eq } = await import("drizzle-orm");
...
export async function getAllCcpRecords(filters?: {
  ...
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const { hCcpInstances, hBatches, hProductsV2 } = await import("../../../drizzle/schema/schema_main");
...
export async function deleteCcpInstances(instanceIds: number[], tenantId?: number) {
  ...
  const instanceCondition = tenantId
    ? and(inArray(hCcpInstances.id, instanceIds), eq(hCcpInstances.tenantId, tenantId as number))
    : inArray(hCcpInstances.id, instanceIds);

  const rowCondition = tenantId
    ? and(inArray(hCcpRows.instanceId, instanceIds), eq(hCcpRows.tenantId, tenantId as number))
...
// CCP 이탈 건수 조회
export async function getCcpDeviationCount(instanceId: number, tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
...
export async function createCcpInstance(data: {
  ...
  if (data.tenantId) values.tenantId = data.tenantId;
  const result = await db.insert(hCcpInstances).values(values as any);
  return Number(result[0].insertId);
}
...
export async function deleteMaterial(id: number, tenantId?: number) {
  ...
  if (tenantId) conditions.push(eq(hMaterials.tenantId, tenantId as number));
  await db
    .update(hMaterials)
    .set({ isActive: 0 })
    .where(and(...conditions));
...
export async function getMonthlyCcpDeviationRate(days: number = 30, tenantId?: number) {
  const db = await getDb();
