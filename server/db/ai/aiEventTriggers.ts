  ...
  try {
    ...
    try {
      ...
      const draft = await generateCorrectiveActionDraft({
        type: deviationType,
        description: `${ccpType} CCP 이탈: ${deviationType} - 측정값 ${measuredValue}, 기준 ${criticalLimit}`,
        ccpType,
        actualValue: measuredValue,
        standardValue: criticalLimit,
      ...
      await conn.execute(
        ...
        [
          ...
          (draft as any).immediateAction || "해당 제품 격리 및 라인 정지",
          (draft as any).rootCauseAnalysis || "원인 조사 중",
          (draft as any).correctiveAction || "시정조치 수립 필요",
          (draft as any).preventiveAction || "재발방지 대책 수립 필요",
        ]
      );
