...
import { hMaterials, hSuppliers } from "../../../drizzle/schema/schema_main.js";
...
export async function generatePurchaseCode(tenantId?: number): Promise<string> {
  ...
  const { accountingPurchases } = await import("../../../drizzle/schema/schema_accounting_extended.js");
...
export async function generateSaleCode(tenantId?: number): Promise<string> {
  ...
  const { accountingSales } = await import("../../../drizzle/schema/schema_accounting_extended.js");
  const { eq } = await import("drizzle-orm");
