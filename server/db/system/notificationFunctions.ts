...
// 검사 결과 부적합 발생 시 알림 생성 (테넌트별 격리)
export async function checkAndCreateInspectionFailureAlerts() {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");

  const { hInspectionRecords, hNotifications, users, tenants } = await import("../../../drizzle/schema.js");
  ...
  for (const tenant of activeTenants) {
    ...
    const failedInspections = await db
      ...
      .where(
        and(
          eq(hInspectionRecords.tenantId, tenantId),
