...
export async function getUsersByRole(role: "admin" | "worker" | "inspector" | "user" | "audit", tenantId?: number) {
  const db = await getDb();
  if (!db) {
...
export async function updateUserRole(userId: number, role: "admin" | "worker" | "monitor" | "employee", tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
...
export async function approveUser(userId: number, role: "admin" | "worker" | "monitor", tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const conditions = [eq(users.id, userId)];
  if (tenantId) conditions.push(eq(users.tenantId, tenantId));
...
export async function toggleUserActive(userId: number, isActive: boolean, tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const conditions = [eq(users.id, userId)];
  if (tenantId) conditions.push(eq(users.tenantId, tenantId));
...
export async function batchApproveUsers(userIds: number[], role: "admin" | "worker" | "monitor", tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const conditions: any[] = [inArray(users.id, userIds)];
  if (tenantId) conditions.push(eq(users.tenantId, tenantId));
  await db.update(users)
    .set({
      approvalStatus: "approved",
...
export async function rejectUser(userId: number, tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const conditions = [eq(users.id, userId)];
  if (tenantId) conditions.push(eq(users.tenantId, tenantId));
...
export async function batchRejectUsers(userIds: number[], tenantId?: number) {
  const db = await getDb();
  if (!db) throw new Error("DB 연결 실패");
  const conditions: any[] = [inArray(users.id, userIds)];
  if (tenantId) conditions.push(eq(users.tenantId, tenantId));
