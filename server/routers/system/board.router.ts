      ...
      try {
        ...
        try {
          const [usersRows]: any = await pool.execute(
            ...
            [Number(ctx.tenantId), Number(ctx.user.id)]
          );
            ...
            try {
              await pool.execute(
                `INSERT INTO communication_log_notifications (tenant_id, log_id, user_id, type, message, is_read)
                 VALUES (?, ?, ?, 'notice', ?, false)`,
                [Number(ctx.tenantId), insertId, Number(u.id), `새로운 ${typeLabel}가 등록되었습니다`]
              );
            } catch (ne: any) {
  ...
  markAlarmRead: tenantRequiredProcedure
    ...
    .mutation(async ({ input, ctx }: any) => {
      ...
      await pool.execute(
        ...
        [Number(input.alarmId), Number(ctx.tenantId)]
      );
      return { success: true };
    }),
  ...
  markAllAlarmsRead: tenantRequiredProcedure
    .mutation(async ({ ctx }: any) => {
      ...
      await pool.execute(
        ...
        [Number(ctx.user.id), Number(ctx.tenantId)]
      );
      return { success: true };
    }),
    ...
    .query(async ({ input, ctx }: any) => {
      ...
      try {
        const [rows]: any = await pool.execute(
          ...
          [Number(input.logId), Number(ctx.tenantId)]
        );
