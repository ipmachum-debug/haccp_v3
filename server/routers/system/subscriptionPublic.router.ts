...
import { tenants, subscriptionNotifications, packageFeatures } from "../../../drizzle/schema/schema_main";
...
export const subscriptionPublicRouter = router({
  ...
  /**
   * 구독 상태 체크 (만료 여부 확인)
   * ✅ P0 FIX: ctx.tenantId 사용 (기존도 ctx.tenantId 사용하여 안전)
   */
  ...
  getSubscriptionStats: superAdminProcedure.query(async ({ ctx }) => {
    ...
    allTenants.forEach((tenant) => {
      if ((tenant.subscriptionPackage as string) === 'basic') package_basic++;
      else if ((tenant.subscriptionPackage as string) === 'pro') package_pro++;
