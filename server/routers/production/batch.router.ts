      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (ccpCreated && ccpGroups.length > 0) {
          try {
            ...
            // BOM에서 batch_target_kg — batchOrchestrator에서 이미 조회했으나
            // batch.router.ts의 단일 배치 경로에서는 별도 조회 필요
            let localBomBatchKg: number | null = null;
            try {
              ...
              if ((bomRows as any[]).length > 0 && (bomRows as any[])[0]?.batch_target_kg) {
                localBomBatchKg = parseFloat((bomRows as any[])[0].batch_target_kg);
              }
            ...
            let batchCount = 1;
            if (localBomBatchKg && localBomBatchKg > 0 && input.plannedQuantity > 0) {
              batchCount = Math.ceil(input.plannedQuantity / localBomBatchKg);
            ...
            for (const grp of ccpGroups) {
              ...
              // grouped 모드: equip_batch_size만큼 병렬 운전 → 실제 배치수 축소
              const equipBatchSize = grp.equip_batch_size ?? 1;
              let grpBatchCount = batchCount;
              if (equipGroupMode === 'grouped' && equipBatchSize > 1) {
                grpBatchCount = Math.ceil(batchCount / equipBatchSize);
                if (grpBatchCount < 1) grpBatchCount = 1;
              }
    ...
    complete: tenantRequiredProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          try {
            const { autoGenerateDailyReport } = await import('../../lib/autoDailyReport');
            dailyReportResult = await autoGenerateDailyReport(input.batchId, ctx.user.id, workDate);
