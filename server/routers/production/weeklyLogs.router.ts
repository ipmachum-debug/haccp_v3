  ...
  getPreviousFormData: tenantRequiredProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        console.error('[weeklyLog.getPreviousFormData]', e);
        return null;
      }
    }),
  ...
  getByDate: tenantRequiredProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        console.error('[weeklyLog.getByDate]', e);
        return null;
      }
    }),
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        if (existingRows.length > 0) {
          recordId = (existingRows[0] as any).id;
            ...
            await db.execute(sql`
              ...
              VALUES
                (${siteId}, ${tenantId}, 'weekly_log', 'checklist', ${recordId},
                 ${`[주간일지] ${input.logDate} 일반위생관리 및 공정점검표`},
                 ${'주간 위생점검 완료 - 승인 요청합니다.'}, 'pending_review', 'medium', ${ctx.user.id}, NOW())
            `);
          } else {
            await db.execute(sql`
        ...
        console.error('[weeklyLog.saveFullForm]', error);
        throw new TRPCError({
  ...
  updateFormData: tenantRequiredProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        console.error('[weeklyLog.updateFormData]', error);
  ...
  list: tenantRequiredProcedure
    ...
    .query(async ({ input = { limit: 50, offset: 0 }, ctx }) => {
      try {
        ...
        console.error('[weeklyLog.list]', error);
        return [];
      }
    }),
