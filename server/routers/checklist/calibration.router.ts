...
export const calibrationRouter = router({
  listEquipments: tenantRequiredProcedure.query(async ({ ctx }) => {
    ...
    const equipments = await db.select().from(calibrationEquipment).where(eq(calibrationEquipment.tenantId, Number(ctx.tenantId))).orderBy(desc(calibrationEquipment.createdAt));
    return equipments;
  }),
