...
export const weeklyLogsRouter = router({
  ...
  deleteHygiene: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (tid) {
        await conn.execute('DELETE FROM weekly_hygiene_logs WHERE id = ? AND tenant_id = ?', [input.id, tid]);
      } else {
  ...
  deletePest: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (tid) {
        await conn.execute('DELETE FROM weekly_pest_logs WHERE id = ? AND tenant_id = ?', [input.id, tid]);
      } else {
