    ...
    update: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const existing = await db.select().from(matchingRules).where(
          and(eq(matchingRules.id, id), eq(matchingRules.tenantId, ctx.tenantId ?? undefined))
        ).limit(1);
        if (!existing.length) {
    ...
    delete: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db.delete(matchingRules).where(
          and(eq(matchingRules.id, input.id), eq(matchingRules.tenantId, ctx.tenantId ?? undefined))
        );
        return { success: true };
      }),
