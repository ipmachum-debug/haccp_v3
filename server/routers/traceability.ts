...
export const traceabilityRouter = router({
   ...
   * 완제품 LOT 번호로 역방향 추적
   */
  byProductLot: protectedProcedure
   ...
   * 원재료 LOT 번호로 정방향 추적
   */
  byMaterialLot: protectedProcedure
    .input(z.object({ lotNumber: z.string() }))
    .query(async ({ input }) => {
   ...
   */
  getTopSearched: protectedProcedure.query(async () => {
   ...
   * 특정 LOT 번호의 추적 이력
   */
  getHistoryByLot: protectedProcedure
    .input(z.object({ lotNumber: z.string() }))
    .query(async ({ input }) => {
   ...
   * LOT 추적 결과 PDF 생성
   */
  generateTracePdf: protectedProcedure
    .input(
      z.object({
        traceType: z.enum(["forward", "backward"]),
        searchLotNumber: z.string(),
