...
import { generateSkuCode, generateExternalProductCode, generateSubsidiaryCode, generateProductCode, generateMaterialCode } from "../../db/system/codeGenerator.js";
  ...
  generateCode: tenantRequiredProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      switch (input.itemType) {
        case "own_product":
          code = await generateProductCode(ctx.tenantId);
          break;
        case "raw_material":
          code = await generateMaterialCode(ctx.tenantId);
          break;
        case "external_product":
          code = await generateExternalProductCode(ctx.tenantId);
          break;
        case "subsidiary":
          code = await generateSubsidiaryCode(ctx.tenantId);
          break;
        default:
          code = "UNKNOWN";
      }
  ...
  create: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!itemCode) {
        switch (input.itemType) {
          case "own_product":
            itemCode = await generateProductCode(ctx.tenantId);
            break;
          case "raw_material":
            itemCode = await generateMaterialCode(ctx.tenantId);
            break;
          case "external_product":
            itemCode = await generateExternalProductCode(ctx.tenantId);
            break;
          case "subsidiary":
            itemCode = await generateSubsidiaryCode(ctx.tenantId);
            break;
          default:
            itemCode = "UNKNOWN";
        }
  ...
  downloadAll: tenantRequiredProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const items = await db.select().from(itemMaster)
        .where(and(eq(itemMaster.tenantId, ctx.tenantId), eq(itemMaster.itemType, input.itemType as any) ) as any)
...
export const productSkuRouter = router({
  ...
  generateCode: tenantRequiredProcedure
    ...
    .query(async ({ input, ctx }) => {
      const code = await generateSkuCode(input.parentItemCode, ctx.tenantId);
      return { code };
    }),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.isDefault === 1) {
        await db.update(productSkus)
          ...
          .where(and(
            eq(productSkus.itemId, input.itemId),
            eq(productSkus.tenantId, ctx.tenantId as any) 
          ));
      }
      ...
      if (!skuCode) {
        ...
        skuCode = await generateSkuCode(parentItem?.itemCode || String(input.itemId), ctx.tenantId);
      }
