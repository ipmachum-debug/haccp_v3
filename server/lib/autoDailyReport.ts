...
export async function autoGenerateDailyReport(
  ...
  try {
    ...
    if (!siteId) throw new Error(`배치 #${batchId}에 siteId가 없습니다`);
    if (!tenantId) throw new Error(`배치 #${batchId}에 tenantId가 없습니다`);

    // 작업일 = 배치의 계획일 기준 (배치 생성일이 아닌 작업 날짜로 일일일지 분류)
    // ★ toISOString()은 UTC 기반이라 KST(+9) 환경에서 날짜가 하루 밀릴 수 있음
    //    → getFullYear/Month/Date (local timezone) 사용
    ...
    if (existingRows.length > 0) {
      ...
      try {
        ...
        if (prevRows.length > 0) {
          ...
          (formData as any).preFilledFrom = prevRows[0].form_date instanceof Date
            ? formatLocalDate(prevRows[0].form_date)
      ...
      `);
      const { getFirstRow } = await import("../utils/dbHelpers");
      ...
      `);
      const { getInsertId } = await import("../utils/dbHelpers");
      checklistRecordId = getInsertId(insertResult);
