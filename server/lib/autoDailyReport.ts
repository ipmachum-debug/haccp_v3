/**
 * 배치 생성/완료 시 일일일지(일반위생관리 및 공정점검표) 자동 생성 서비스
 *
 * 워크플로우:
 ...
 *    → 모든 배치 삭제 시: 자동생성 일일일지 레코드 자체 삭제 → 배치 재생성 시 새로 생성
 * ※ form_data 구조: 기존 DailyLogForm.tsx / DB record(id=90) 구조와 동일
 * ※ HACCP 체크리스트 > 일일일지 목록에서 genericChecklist.list(formType='daily_log')로 관리
 */

import { getDb } from "../db";
import { sql, eq } from "drizzle-orm";

/** plannedDate(UTC) → KST YYYY-MM-DD 변환 (서버 TZ와 무관하게 항상 +09:00) */
...
export async function autoGenerateDailyReport(
  ...
  try {
    ...
    // 작업일 = 배치의 계획일 기준 (KST 변환 - UTC+9)
    // planned_date는 UTC로 저장 (예: 2026-02-27T15:00:00Z = 2026-02-28 KST)
    const batchPlannedDate = toKstDateStr(batch.plannedDate as any);
    const reportDate = batchPlannedDate || toKstDateStr(new Date()) || new Date().toISOString().split("T")[0];
    console.log(`[autoDailyReport] 배치 #${batchId} plannedDate=${batch.plannedDate} → KST reportDate=${reportDate}`);
    ...
    if (existingRows.length > 0) {
      ...
      `);
      console.log(`[autoDailyReport] 기존 일일일지 #${checklistRecordId}에 배치 #${batchId} 추가 (총 ${formData.batches.length}건)`);
