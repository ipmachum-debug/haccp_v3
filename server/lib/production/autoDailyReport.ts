  ...
  try {
    ...
    if (existingRows.length > 0) {
      ...
      await db.execute(sql`
        ...
        WHERE id = ${checklistRecordId} AND tenant_id = ${tenantId}
      `);
      console.log(`[autoDailyReport] 기존 일일일지 #${checklistRecordId}에 배치 #${batchId} 추가`);

      // ★ 자기치유(self-heal): 기존 record 의 짝꿍 AR 이 없으면 보충 INSERT.
      //   원인 시나리오:
      //     - record 가 다른 경로(수동/cron) 로 먼저 생성되어 짝꿍 AR 이 없는 고아 상태
      //     - 이후 배치 생성 시 이 분기로 들어와 form_data 만 갱신되고 AR 은 영원히 미생성
      //   결과: 승인관리 화면에 일일일지가 보이지 않음 (record 는 존재하지만 AR 없음)
      //   대응: existingAR 이 없으면 즉시 보충하여 record↔AR 1:1 invariant 유지.
      try {
        const arCheck = await db.execute(sql`
          SELECT id FROM h_approval_requests
          WHERE tenant_id = ${tenantId}
            AND request_type = 'daily_log'
            AND reference_type = 'checklist'
            AND reference_id = ${checklistRecordId}
          LIMIT 1
        `);
        const arRows = (arCheck as any)[0] || [];
        if (arRows.length === 0) {
          const recoverTitle = `[일일일지] ${reportDate} 일반위생관리 및 공정점검표`;
          const recoverDesc =
            `작업일: ${reportDate}\n` +
            `[자가복구 — record_id=${checklistRecordId} 의 짝꿍 AR 누락 감지 → 자동 보충]`;
          const recoverInsert = await db.execute(sql`
            ...
            VALUES
              (${siteId}, ${tenantId}, 'daily_log', 'checklist', ${checklistRecordId},
               ${recoverTitle}, ${recoverDesc}, 'pending_review', 'medium', ${userId}, NOW())
          `);
          approvalRequestId = Number((recoverInsert as any)[0]?.insertId || 0);
          console.warn(
            `[autoDailyReport] ★자가복구★ 고아 record #${checklistRecordId} 의 짝꿍 AR 자동 보충: AR #${approvalRequestId}`,
          );
        }
      } catch (healErr: any) {
        console.error(
          `[autoDailyReport] 짝꿍 AR 자가복구 실패 (record #${checklistRecordId}, 계속):`,
