...
export async function postPurchase(purchaseId: number, userId: number): Promise<{ alreadyProcessed: boolean }> {
  ...
  return await withTransaction(async (conn) => {
    ...
    // ★ PR-§5.2-2 (2026-04-27): material_id 직접 작성 — resolvedMaterialId 사용.
    //   매입 확정 시 LOT 의 material_id 와 동일 (purchasePost.ts 내 LOT 생성도 같은 값).
    await conn.execute(
      ...
      [tenantId, lotId, resolvedMaterialId, qty, purchase.unit || "EA", purchase.transactionDate,
