...
const CHECKLIST_PROMPTS: Record<string, PromptConfig> = {
  ...
  // ★ PR-AV (2026-05-28): row별 productName 추출 (행마다 다른 제품 지원)
  //   + 손글씨 연도 4자리 판독 강화 + 압력 8/0 혼동 사례 더 명시.
  ccp_1b: {
...
공통 필드 (영역 A 에서):
  - recordDate: 작성일자 (YYYY-MM-DD).
    ⚠️ 손글씨 연도 4자리를 모두 정확히 판독할 것. 끝자리만 보고 추정 금지.
    ⚠️ 한국 손글씨 "2026" / "2025" / "2024" 모두 가능 — 보이는 그대로 4자리 모두 판독.
    ⚠️ "2023" 처럼 과거 연도로 보이면 한 번 더 확인 — 손글씨 "6" 이 "3" 으로 보일 수 있음.
    예: "2026년 5월 28일" → "2026-05-28"
  - inspector: 작성자 (영역 A 우측 박스 또는 영역 C 의 작성자 컬럼)

measurements[] 배열 (영역 C 의 손글씨 데이터 표의 각 행):
  ⚠️ 매우 중요: 각 측정 행은 서로 다른 제품일 수 있습니다 (인절미, 콩고물쑥떡, 찹쌀떡 등).
  ⚠️ 반드시 각 행의 품명 컬럼을 독립적으로 판독하여 productName 에 넣을 것.
  ⚠️ 첫 행의 품명을 다른 행에 복사하지 말 것 — 행마다 다를 수 있음.

  필드:
  - productName: 이 행의 품명 컬럼 손글씨 (예: "인절미", "콩고물쑥떡", "찹쌀떡", "약식").
    ⚠️ 영역 B (한계기준 표) 의 "찹쌀떡류(교반기1호기)" 같은 인쇄 텍스트는 절대 사용 금지.
    ⚠️ 영역 C 측정 데이터 표의 손글씨 품명만 사용.
    ⚠️ 같은 PDF 안에서 행마다 다른 품명일 수 있음 — 각 행 독립 판독.
  - measurementTime: 측정시각 (HH:MM). 손글씨 손에 익은 글씨 — "06:00", "06:15" 같은 형식 다수.
    ⚠️ "06:10" 과 "06:00", "06:15" 와 "06:11", "06:25" 와 "06:35" 를 신중히 구별. 분 단위 두 자리 정확히.
  ...
  - pressureMpa: 압력 (소수 2자리, 예: "0.18", "0.16", "0.12", "0.10", "0.28").
    ⚠️ 매우 중요: 손글씨 "0.18" 과 "0.10" 의 끝자리 8 vs 0 구별이 가장 빈번한 오류.
    ⚠️ 한계기준 표에 "0.16~0.20" 같은 범위가 있으면 손글씨가 그 범위 안일 가능성 높음.
    ⚠️ 0.10 은 실제 운영에서 드뭄 — 0.10 으로 판독되면 "8" 을 "0" 으로 잘못 본 건 아닌지 한 번 더 확인.
    ⚠️ 영역 B (한계기준) 의 "0.16Mpa이상" 텍스트를 추출하지 말 것. 손글씨 값만.
  - inputAmountKg: 투입량 (kg, 정수 또는 소수).
    ⚠️ 손글씨 5와 6, 0과 6, 3과 8, 2와 8 혼동 빈번. 매우 신중히 판독.
    ⚠️ 예: 56 ↔ 66, 38 ↔ 28, 60 ↔ 66 — 글자 모양의 굴곡을 잘 보고 구별.
...
═══ 손글씨 판독 일반 가이드 ═══
  - 손글씨 숫자 0 과 6, 5 와 6, 1 과 7, 8 과 0, 3 과 8, 2 와 8 을 매우 신중히 구별.
...
한 PDF = 한 작성일 = 여러 측정 (각각 다른 제품 가능). 각 측정 행은 독립된 DB 레코드가 됩니다.`,
    jsonSchema: `{
  "recordDate": "YYYY-MM-DD",
  "inspector": "작성자",
  "measurements": [
    {
      "productName": "이 행의 품명 (예: 인절미, 콩고물쑥떡, 찹쌀떡)",
      "measurementTime": "HH:MM",
      "equipment": "교반기1호기 또는 1, 2, 3 등",
      "heatingTimeMin": 숫자,
      "pressureMpa": "0.180",
      "inputAmountKg": "56",
      "tempEdgeC": "99",
  ...
  // ★ PR-AU (2026-05-28): 양식 영역 명확 구분 + 핵심 오류 패턴 경고 강화.
  // ★ PR-AV (2026-05-28): row별 productName + 연도 4자리 판독 강화.
  ccp_2b: {
...
공통 필드 (영역 A):
  - recordDate: 작성일자 (YYYY-MM-DD)
    ⚠️ 손글씨 연도 4자리를 모두 정확히 판독. 끝자리만 보고 추정 금지.
    ⚠️ "2026" / "2025" / "2024" 모두 가능 — 보이는 그대로.
    ⚠️ "2023" 처럼 과거 연도로 보이면 한 번 더 확인 — "6" 이 "3" 으로 보일 수 있음.
  - inspector: 작성자

measurements[] 배열 (영역 C 손글씨 데이터 표의 각 행):
  ⚠️ 매우 중요: 각 측정 행은 서로 다른 제품일 수 있습니다 (마카다미아, 호두, 아몬드 등).
  ⚠️ 반드시 각 행의 품명 컬럼을 독립적으로 판독하여 productName 에 넣을 것.

  필드:
  - productName: 이 행의 품명 컬럼 손글씨 (예: "마카다미아", "호두", "아몬드").
    ⚠️ 영역 B (한계기준 표) 의 인쇄 텍스트 절대 사용 금지.
    ⚠️ 영역 C 측정 데이터 표의 손글씨 품명만 사용. 행마다 다를 수 있음.
  ...
  - inputAmountKg: 투입량 (kg, 정수 또는 소수).
    ⚠️ 5↔6, 8↔0, 1↔7, 3↔8, 2↔8 손글씨 혼동 빈번.
...
한 PDF = 한 작성일 = 여러 측정. 각 행은 독립 DB 레코드.`,
  ...
  "measurements": [
    {
      "productName": "이 행의 품명 (예: 마카다미아, 호두, 아몬드)",
      "measurementTime": "HH:MM",
      "heatingTimeMin": 숫자,
      "temperatureC": "150.0",
      "inputAmountKg": "50.00",
