/**
 * CcpMonitoringForms 분해 — FormPanel 섹션.
 * Constants + helpers (ResultBadge, OXCell, BatchCountCalc, EquipmentAssignSection)
 * + RowInputForm + CcpFormPanel.
 */
...
import {
  ...
  AlertTriangle, Loader2, Clock, Calculator, Wrench,
} from "lucide-react";
import { cn } from "../../../lib/utils";

export const CCP_TYPE_LABELS: Record<string, string> = {
  "CCP-2B": "가열(굽기)공정",
  "CCP-1B": "가열(증숙)공정",
  "CCP-4P": "금속검출공정",
};
...
// CL 기준값 (기준서 기반)
export const CCP_CL_DEFAULTS: Record<string, {
...
export function OXCell({ value, onChange }: { value?: string | null; onChange?: (v: string) => void }) {
  if (!onChange) {
    return (
...
// BOM 배치수 계산 섹션
// ─────────────────────────────────────────────────────────────────────────────
export function BatchCountCalc({
...
// 설비 할당 섹션 (배치 인터벌 포함)
// ─────────────────────────────────────────────────────────────────────────────
export function EquipmentAssignSection({
