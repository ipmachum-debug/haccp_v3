...
import { Eye, Trash2, CheckCircle, XCircle, Clock, FileDown } from 'lucide-react';

interface MonthlyHygieneLogListModalProps {
  open: boolean;
  onClose: () => void;
  tenantId: number;
...
export function MonthlyHygieneLogListModal({
  open,
  onClose,
  tenantId,
  onViewDetail
}: MonthlyHygieneLogListModalProps) {
  const { toast } = useToast();
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | '작성중' | '승인대기' | '승인완료'>('all');

  const { data, isLoading, refetch } = trpc.monthlyLog.getHygiene.useQuery(
    {
      tenant_id: tenantId,
  ...
  const deleteMutation = trpc.monthlyLog.deleteHygiene.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const requestApprovalMutation = trpc.monthlyLog.requestHygieneApproval.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const rejectMutation = trpc.monthlyLog.rejectHygiene.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
        description: '반려되었습니다.'
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>청소 상태</TableHead>
                  <TableHead>위생교육</TableHead>
                  <TableHead>CCP 검증</TableHead>
                ...
                {isLoading ? (
                  ...
                  data.logs.map((log: any) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>{log.cleaning_status || '-'}</TableCell>
                      <TableCell>{log.education_status || '-'}</TableCell>
                      <TableCell>{log.ccp_verification || '-'}</TableCell>
