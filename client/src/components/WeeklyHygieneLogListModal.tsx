...
import { WeeklyHygienePDFExport } from './WeeklyHygienePDFExport';

interface WeeklyHygieneLogListModalProps {
  open: boolean;
  onClose: () => void;
  tenantId: number;
...
export function WeeklyHygieneLogListModal({
  open,
  onClose,
  tenantId,
  onViewDetail
}: WeeklyHygieneLogListModalProps) {
  const { toast } = useToast();
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | '작성중' | '승인대기' | '승인완료'>('all');

  const { data, isLoading, refetch } = trpc.weeklyLog.getHygiene.useQuery(
    {
      tenant_id: tenantId,
  ...
  const deleteMutation = trpc.weeklyLog.deleteHygiene.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const requestApprovalMutation = trpc.weeklyLog.requestHygieneApproval.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const rejectMutation = trpc.weeklyLog.rejectHygiene.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
        description: '반려되었습니다.'
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>냉장창고 청소</TableHead>
                  <TableHead>작업장 청소</TableHead>
                  <TableHead>위생복 세탁</TableHead>
                ...
                {isLoading ? (
                  ...
                  data.logs.map((log: any) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>{log.cold_storage_clean || '-'}</TableCell>
                      <TableCell>{log.facility_clean || '-'}</TableCell>
                      <TableCell>{log.uniform_wash || '-'}</TableCell>
                      <TableCell>{getStatusBadge(log.status)}</TableCell>
                      <TableCell>{log.approved_by || '-'}</TableCell>
                      <TableCell>
                        <div className="flex gap-2 justify-center">
                          <WeeklyHygienePDFExport log={log} />
                          {onViewDetail && (
                            <Button
                              size="sm"
                              variant="outline"
