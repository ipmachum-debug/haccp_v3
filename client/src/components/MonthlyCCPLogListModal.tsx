...
interface MonthlyCCPLogListModalProps {
  open: boolean;
  onClose: () => void;
  tenantId: number;
...
export function MonthlyCCPLogListModal({
  open,
  onClose,
  tenantId,
  onViewDetail
}: MonthlyCCPLogListModalProps) {
  const { toast } = useToast();
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | '작성중' | '승인대기' | '승인완료'>('all');

  const { data, isLoading, refetch } = trpc.monthlyLog.getCCP.useQuery(
    {
      tenant_id: tenantId,
  ...
  const deleteMutation = trpc.monthlyLog.deleteCCP.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const approveMutation = trpc.monthlyLog.approveCCP.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const requestApprovalMutation = trpc.monthlyLog.requestCCPApproval.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
  ...
  const rejectMutation = trpc.monthlyLog.rejectCCP.useMutation({
    onSuccess: () => {
      toast({
        title: '성공',
        description: '반려되었습니다.'
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>가열 공정</TableHead>
                  <TableHead>금속검출</TableHead>
                ...
                {isLoading ? (
                  ...
                  data.logs.map((log: any) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        {log.heating_temp_time_check === '예' ? '✓' : log.heating_temp_time_check === '아니오' ? '✗' : '-'}
                      </TableCell>
                      <TableCell>
                        {log.metal_detector_test === '예' ? '✓' : log.metal_detector_test === '아니오' ? '✗' : '-'}
                      </TableCell>
                      <TableCell>{log.action_taker || '-'}</TableCell>
