...
export default function ChecklistFormLayout({
  ...
  // ============================================================================
  // 새 문서 작성 시 이전 작성 내용 자동 복원 (프리필)
  // ★ 2026-04-13 버그 수정:
  //    기존: setSavedRecordId(latestByDate.id) 로 업데이트 모드 전환
  //         → 저장 시 이전 레코드가 덮어써져서 과거 이력이 사라짐!
  //    수정: savedRecordId 는 null 유지 → Save 클릭 시 CREATE (신규 레코드)
  //    또한 날짜/결재/ID 등 메타 필드는 pre-fill 에서 제외하여
  //    사용자가 방금 선택한 '오늘' 날짜를 유지하도록 함
  // ============================================================================
  useEffect(() => {
    if (!isEdit && latestByDate && latestByDate.formData && !dataLoaded && approvalSettingApplied) {
      const fd = latestByDate.formData as any;
      try {
        // ★ 데이터 필드만 pre-fill 하기 위해 메타 필드를 제거한 복사본 생성
        // - formDate / checkDate / date: 현재 날짜 유지 (과거 날짜 덮어쓰기 방지)
        // - id / approval / status / createdAt / updatedAt: 새 레코드의 상태 유지
        const prefillData = { ...fd };
        delete prefillData.formDate;
        delete prefillData.checkDate;
        delete prefillData.date;
        delete prefillData.inspectionDate;
        delete prefillData.workDate;
        delete prefillData.reportDate;
        delete prefillData.id;
        delete prefillData.approval;
        delete prefillData.status;
        delete prefillData.createdAt;
        delete prefillData.updatedAt;

        // 각 폼 고유 데이터 복원 (입력 필드 내용만)
        if (onDataRestore) {
          onDataRestore(prefillData);
        }
        // ★ savedRecordId 를 null 유지 → 저장 시 신규 레코드 생성
        setDataLoaded(true);
        toast({
          title: "이전 값 자동 불러오기",
          description: "가장 최근 저장 값을 프리필했습니다. 수정 후 저장하면 새 레코드로 기록됩니다.",
        });
      } catch (e) {
