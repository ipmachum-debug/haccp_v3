      ...
      {batchData.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">{`${L("batch")} 생산 정보 (자동 연동)`}</CardTitle>
            <CardDescription>{logDate} 배치 {batchData.length}건</CardDescription>
          </CardHeader>
          <CardContent>
            <table className="w-full border-collapse border text-sm">
              <thead><tr className="bg-muted">
                <th className="border p-1">{`${L("batch")}코드`}</th><th className="border p-1">{`${L("product")}명`}</th>
      ...
      {isLoading ? (
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="temperature" className="space-y-4">
            <Card>
              <CardHeader><CardTitle>{`${L("material")}실 온/습도 점검기록지`}</CardTitle><CardDescription>온도: 1~35 C, 습도: 65% 이하</CardDescription></CardHeader>
              <CardContent className="space-y-4">
                <table className="w-full border-collapse border text-sm">
                  <thead><tr className="bg-muted">
          ...
          <TabsContent value="refrigerator" className="space-y-4">
            <Card>
              <CardHeader><CardTitle>{`${L("material")} 냉장고 온도 점검 기록지`}</CardTitle><CardDescription>온도: 0~10 C</CardDescription></CardHeader>
              <CardContent className="space-y-4">
                <table className="w-full border-collapse border text-sm">
                  <thead><tr className="bg-muted">
                    <th className="border p-2">시간</th><th className="border p-2">검사시각</th>
