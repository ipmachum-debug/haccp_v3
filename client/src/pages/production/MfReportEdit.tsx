      ...
      <div className="space-y-6 pb-24">
          ...
          <Card className="mb-3">
            <CardContent className="p-3">
              <div className="grid grid-cols-4 gap-3 items-end">
                <div>
                  <Label className="text-xs text-muted-foreground mb-1">{`${L("product")}`}</Label>
                ...
                <div>
                  <Label className="text-xs text-muted-foreground mb-1 font-semibold text-primary">{`${L("batch")} 총생산량(kg)`}</Label>
          ...
          <Card className="mb-3">
            ...
            <CardContent className="px-0 pb-0">
              {ingredients.length === 0 ? (
                <div className="text-center py-6 text-muted-foreground text-sm">{`${L("material")}가 없습니다`}</div>
              ) : (
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-muted/60 border-y text-xs text-muted-foreground">
                        ...
                        <th className="px-3 py-2 text-left" style={{minWidth: "180px"}}>{`${L("material")}`}</th>
                        ...
                        <th className="px-3 py-2 text-right" style={{width: "100px"}}>
                          <TooltipProvider>
                            <Tooltip>
                              ...
                              <TooltipContent><p className="text-xs">{`${L("batch")} 총생산량 × 비율(%) 자동 환산`}</p></TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                        </th>
              ...
              {additionalIngredients.length === 0 ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-muted/60 border-y text-xs text-muted-foreground">
                      <th className="px-3 py-2 text-left" style={{width: "36px"}}>#</th>
                      <th className="px-3 py-2 text-left" style={{width: "auto"}}>{`${L("material")}`}</th>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3">
                      ...
                      <span>{L("batch")} 목표: <strong>{batchProdKg.toFixed(1)}kg</strong></span>
                      <span className="text-muted-foreground">|</span>
          ...
          <div className="flex justify-between items-center">
            <div className="flex gap-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={handleBatchPreview}>
                <Eye className="w-4 h-4 mr-1" />{`${L("batch")} 계산
              `}</Button>
            </div>
            <div className="flex gap-2">
        ...
        <Dialog open={batchPreviewOpen} onOpenChange={setBatchPreviewOpen}>
          <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{`${L("batch")} 계산 미리보기`}</DialogTitle>
              <DialogDescription>{`${L("batch")} 크기에 따른 원재료 필요량 (모든 단위: kg)`}</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <Label className="text-xs mb-1">{`${L("batch")} 크기 (kg)`}</Label>
