...
function SalesManagementContent() {
  ...
  // ★ 2026-05-16: 저장 후 화면 유지 + 인쇄 흐름
  //   기존엔 저장 즉시 폼 초기화 → 사용자가 거래명세표 출력 불가 사고.
  //   이제 방금 저장한 sale.id 들을 보관하고, 거래명세표 인쇄 / 새 매출 등록 버튼을 노출.
  const [savedSaleIds, setSavedSaleIds] = useState<number[]>([]);
  ...
  // ★ 2026-05-16: createSale onSuccess 에서 자동 폼 초기화 제거.
  //   handleSave 가 모든 행을 일괄 처리한 뒤 한 번에 success/fail 카운팅 + saleId 수집 → 거래명세표 인쇄 흐름 활성.
  ...
  const printSinglePdfMutation = (trpc as any).haccpIntegration.generateSalePDF.useMutation();
  const printGroupPdfMutation = (trpc as any).haccpIntegration.generateSaleGroupPDF.useMutation();
  ...
  const handlePrintStatement = async () => {
    ...
    setIsPrinting(true);
    try {
      const result: any = savedSaleIds.length === 1
        ? await printSinglePdfMutation.mutateAsync({ saleId: savedSaleIds[0] })
        : await printGroupPdfMutation.mutateAsync({ saleIds: savedSaleIds });
      ...
      // 새 창에서 열어 인쇄/다운로드 둘 다 가능 — 팝업 차단 시 다운로드 폴백
  ...
  const handleSave = () => {
    ...
    setIsSaving(true);
    setSavedSaleIds([]); // 재저장 대비 초기화

    let successCount = 0;
    let failCount = 0;
    const newSaleIds: number[] = [];
    ...
    items.forEach((item) => {
      createMutation.mutate(
        ...
        },
        {
          onSuccess: (data: any) => {
            successCount++;
            // ★ 2026-05-16: drizzle/mysql2 insert 결과에서 sale.id 수집
            //   createSale 은 첫 행 (ResultSetHeader / 또는 sale row) 을 반환.
            //   insertId / id 양쪽 모두 시도.
            const insertedId = Number(data?.insertId ?? data?.id ?? 0);
            if (insertedId > 0) newSaleIds.push(insertedId);

            if (successCount + failCount === items.length) {
              setIsSaving(false);
              utils.haccpIntegration.getAllSales.invalidate();
              setSavedSaleIds(newSaleIds);
              toast({
                title: `매출 등록 완료`,
                description: `성공: ${successCount}건${failCount > 0 ? `, 실패: ${failCount}건` : ""}`,
              });
              // ★ 폼은 유지 — 거래명세표 출력 후 "새 매출 등록" 으로 수동 초기화.
            }
          },
          onError: () => {
            failCount++;
  ...
  return (
    <div className="space-y-0">
      ...
      {/* ★ 2026-05-16: 저장 후 결과 + 거래명세표 출력 / 새 매출 등록 버튼 바 */}
        ...
        <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-semibold text-emerald-900">
              {savedSaleIds.length}건 등록 완료
            </span>
            <span className="text-emerald-700 text-xs">
              · 거래명세표를 출력하거나 새 매출을 등록하세요
            </span>
          </div>
