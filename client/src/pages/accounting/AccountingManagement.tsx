...
import { Plus, Download, TrendingUp, TrendingDown, DollarSign, Filter, Edit, Trash2, BarChart3, FileText } from "lucide-react";
...
function categoryLabel(category: string): string {
  switch (category) {
    ...
    case "liabilities": return "부채";
    case "equity": return "자본";
    case "revenue": return "수익";
    case "expenses": return "비용";
...
function AccountingManagementContent() {
  ...
  const [transactionForm, setTransactionForm] = useState({
    ...
    categoryId: "",
    description: "",
    paymentMethod: "cash" as "cash" | "bank" | "card" | "unpaid",
  });
  ...
  const handleEdit = (transaction: any) => {
    ...
    setTransactionForm({
      ...
      paymentMethod: "cash", // 기존 분개에서는 결제수단 정보가 없으므로 기본값 사용
    });
  ...
  // 카테고리 필터링 (수입/지출 구분)
  // income -> revenue 계정만, expense -> expenses 계정만 (자산/부채/자본은 고급 분개에서 사용)
  const filteredCategories = useMemo(() => {
    if (transactionForm.type === "income") {
      return categories.filter((cat: any) => cat.category === "revenue");
    }
    return categories.filter((cat: any) => cat.category === "expenses");
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: "cash" | "bank" | "card" | "unpaid") =>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="bank">은행 이체</SelectItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {categories.map((cat: any) => (
                        <SelectItem key={cat.id} value={String(cat.id)}>
                          [{cat.code}] {cat.name} ({categoryLabel(cat.category)})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
