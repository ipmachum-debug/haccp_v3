...
function generatePrintHTML(items: FinishedProductItem[], year: number, month: number, approvalInfo: any) {
  ...
  for (let p = 0; p < totalPages; p++) {
    ...
    <div class="page-container${p > 0 ? ' page-break' : ''}">
      ...
      <div class="footer">HACCP-ONE | ${year}\uB144 ${month}\uC6D4 \uC644\uC81C\uD488 \uCD9C\uACE0\uAC80\uC0AC\uC77C\uC9C0${totalPages > 1 ? ` (${p + 1}/${totalPages})` : ''}</div>
    </div>`;
  }
  ...
  return (
    <div className="bg-white max-w-full overflow-x-auto space-y-6" style={{ fontFamily: "'Noto Sans KR', 'Malgun Gothic', sans-serif" }}>
      ...
      <table className="w-full border-collapse border border-gray-600 text-[9px]">
        <thead>
          <tr>
            ...
            <th className={thCls} style={{minWidth:'120px'}}>제품명</th>
      ...
      <div className="mt-3 text-[9px] text-gray-400 text-center border-t border-gray-200 pt-2">
        HACCP-ONE | {year}년 {month}월 완제품 출고검사일지
      </div>
    </div>
  );
}
            ...
            <table className="w-full border-collapse text-xs min-w-[900px]">
              <thead>
                <tr className="bg-green-50">
                  ...
                  <th className="border border-gray-300 px-1 py-1.5 text-center min-w-[120px]">제품명</th>
              ...
              <tbody>
                {editItems.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="border border-gray-200 p-0.5"><Input className="h-7 text-xs" value={item.productName} placeholder="제품명" onChange={e => updateItem(idx, 'productName', e.target.value)} /></td>
