...
function renderCcpFormRecord(fr: any, doc: any) {
  ...
  const batchId = s(fr.batchId || fr.batch_id || doc?.referenceId);
  ...
  const clMetalFe = fr.clMetalFenessMm ?? fr.cl_metal_feness_mm;
  const clMetalSus = fr.clMetalSusMm ?? fr.cl_metal_sus_mm;

  // CCP 타입별 한글명
  const ccpTypeLabels: Record<string, string> = {
    "CCP-1B": "가열(교반)공정 CCP 모니터링 기록지",
    "CCP-2B": "가열(굽기)공정 CCP 모니터링 기록지",
    "CCP-4P": "금속검출공정 CCP 모니터링 기록지",
  };

  // 상태별 배지
  const statusLabel = status === "approved" ? "승인완료" : status === "submitted" ? "검토중" : "작성중";
  const statusColor = status === "approved" ? "#16a34a" : status === "submitted" ? "#2563eb" : "#d97706";

  return (
    <div>
      <div className="text-center mb-4">
        <h2 className="text-xl font-bold">{ccpTypeLabels[ccpType] || `${ccpType} CCP 모니터링 기록지`}</h2>
        <p className="text-gray-600 text-sm mt-1">(배 치 작 성)</p>
      </div>
      ...
      <table className="w-full border-collapse border border-gray-400 text-sm mb-4">
        <tbody>
          <tr>
            <td className="border border-gray-400 px-3 py-1.5 bg-gray-50 font-medium w-1/4">제품명</td>
            <td className="border border-gray-400 px-3 py-1.5">{productName || "-"}</td>
            <td className="border border-gray-400 px-3 py-1.5 bg-gray-50 font-medium w-1/4">작업일자</td>
            <td className="border border-gray-400 px-3 py-1.5">{workDate || "-"}</td>
          </tr>
          <tr>
            <td className="border border-gray-400 px-3 py-1.5 bg-gray-50 font-medium">공정</td>
            <td className="border border-gray-400 px-3 py-1.5">{processGroupName || "-"}</td>
            <td className="border border-gray-400 px-3 py-1.5 bg-gray-50 font-medium">CCP 유형</td>
            <td className="border border-gray-400 px-3 py-1.5">
              <span className="font-bold" style={{ color: statusColor }}>{ccpType}</span>
              {" "}
              <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: statusColor + "20", color: statusColor }}>{statusLabel}</span>
            </td>
          </tr>
          <tr>
            <td className="border border-gray-400 px-3 py-1.5 bg-gray-50 font-medium">배치코드</td>
            <td className="border border-gray-400 px-3 py-1.5" colSpan={3}>배치 #{batchId}</td>
          </tr>
        </tbody>
      </table>
      ...
      {/* 한계기준(CL) */}
      <div className="mb-4">
        <div className="bg-blue-50 border border-blue-200 rounded px-3 py-2 text-sm font-semibold mb-2 text-blue-800">
          ■ 한계기준 (CL, Critical Limit)
        </div>
        <table className="w-full border-collapse border border-gray-400 text-sm">
          <thead>
            <tr className="bg-gray-50">
              {ccpType !== "CCP-4P" && <th className="border border-gray-400 px-2 py-1.5">가열시간(분)</th>}
              {ccpType !== "CCP-4P" && <th className="border border-gray-400 px-2 py-1.5">가열온도(℃)</th>}
              {(ccpType === "CCP-1B") && <th className="border border-gray-400 px-2 py-1.5">압력(MPa)</th>}
              {ccpType === "CCP-4P" && <th className="border border-gray-400 px-2 py-1.5">Fe 검출감도(mm)</th>}
              {ccpType === "CCP-4P" && <th className="border border-gray-400 px-2 py-1.5">SUS 검출감도(mm)</th>}
            </tr>
          </thead>
          <tbody>
            <tr>
              {ccpType !== "CCP-4P" && (
                <td className="border border-gray-400 px-2 py-1.5 text-center">
                  {clHeatTimeLo != null ? `≥${clHeatTimeLo}분` : "-"}
                  {clHeatTimeHi != null ? ` ~ ≤${clHeatTimeHi}분` : ""}
                </td>
              )}
              {ccpType !== "CCP-4P" && (
                <td className="border border-gray-400 px-2 py-1.5 text-center">
                  {clHeatTempLo != null ? `≥${clHeatTempLo}℃` : "-"}
                </td>
              )}
              {(ccpType === "CCP-1B") && (
                <td className="border border-gray-400 px-2 py-1.5 text-center">
                  {clPressureMpaLo != null ? `≥${clPressureMpaLo} MPa` : "-"}
                </td>
              )}
              {ccpType === "CCP-4P" && (
                <td className="border border-gray-400 px-2 py-1.5 text-center">
                  {clMetalFe != null ? `φ${clMetalFe}mm 이하` : "규격 시편"}
                </td>
              )}
              {ccpType === "CCP-4P" && (
                <td className="border border-gray-400 px-2 py-1.5 text-center">
                  {clMetalSus != null ? `φ${clMetalSus}mm 이하` : "규격 시편"}
                </td>
              )}
            </tr>
          </tbody>
      ...
      {/* 모니터링 기록 */}
      <div className="mb-4">
        <div className="bg-green-50 border border-green-200 rounded px-3 py-2 text-sm font-semibold mb-2 text-green-800">
          ■ 모니터링 기록
        </div>
        {(() => {
          // form rows 데이터가 있으면 실제 값으로 렌더링
          const formRows: any[] = fr.rows || [];
          const hasFormRows = formRows.length > 0;
          // form rows가 없고 status가 draft면 빈 메시지 표시, 아니면 설비기준 테이블
          if (!hasFormRows && status === "draft") {
            return (
              <div className="border border-dashed border-gray-300 rounded p-4 text-center text-gray-500 text-sm">
                아직 기록이 작성되지 않았습니다.
                <br />배치 상세 페이지에서 CCP 모니터링 기록을 입력해 주세요.
              </div>
            );
          }
          // 실제 데이터로 테이블 렌더링 (formRows 또는 빈 3행 폴백)
          const displayRows = hasFormRows ? formRows : [1, 2, 3].map(seq => ({ batchSeq: seq }));
          ...
          if (ccpType === "CCP-4P") {
            ...
            return (
              <table className="w-full border-collapse border border-gray-400 text-sm">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="border border-gray-400 px-2 py-1.5">순번</th>
                    <th className="border border-gray-400 px-2 py-1.5">설비</th>
                    <th className="border border-gray-400 px-2 py-1.5">검출항목</th>
                    <th className="border border-gray-400 px-2 py-1.5">판정</th>
                ...
                <tbody>
                  {displayRows.map((row: any, idx: number) => (
                    <tr key={idx}>
                      <td className="border border-gray-400 px-2 py-2 text-center">{row.batchSeq ?? row.batch_seq ?? (idx + 1)}</td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{s(row.equipmentName || row.equipment_name) || "-"}</td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{s(row.note) || "-"}</td>
                      <td className="border border-gray-400 px-2 py-2 text-center">
                        <span className={`font-medium ${(row.result === "적합" || row.result === "PASS") ? "text-green-600" : row.result === "부적합" ? "text-red-600" : ""}`}>
                          {row.result || "-"}
                        </span>
                      </td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{s(row.specialNote || row.special_note) || "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          return (
            <table className="w-full border-collapse border border-gray-400 text-sm">
              <thead>
                <tr className="bg-gray-50">
                  <th className="border border-gray-400 px-2 py-1.5">배치순번</th>
                  <th className="border border-gray-400 px-2 py-1.5">설비</th>
                  <th className="border border-gray-400 px-2 py-1.5">측정온도(℃)</th>
                  <th className="border border-gray-400 px-2 py-1.5">가열시간(분)</th>
                  {ccpType === "CCP-1B" && <th className="border border-gray-400 px-2 py-1.5">압력(MPa)</th>}
                  <th className="border border-gray-400 px-2 py-1.5">판정</th>
                  <th className="border border-gray-400 px-2 py-1.5">비고</th>
                </tr>
              ...
              <tbody>
                {displayRows.map((row: any, idx: number) => {
                  ...
                  return (
                    <tr key={idx}>
                      <td className="border border-gray-400 px-2 py-2 text-center">{row.batchSeq ?? row.batch_seq ?? (idx + 1)}배치</td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{s(row.equipmentName || row.equipment_name) || "-"}</td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{heatTemp != null ? `${heatTemp}℃` : "-"}</td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{heatTime != null ? `${heatTime}분` : "-"}</td>
                      {ccpType === "CCP-1B" && <td className="border border-gray-400 px-2 py-2 text-center">{pressure != null ? `${pressure} MPa` : "-"}</td>}
                      <td className="border border-gray-400 px-2 py-2 text-center">
                        <span className={`font-medium ${isPass ? "text-green-600" : result === "부적합" ? "text-red-600" : ""}`}>
                          {result || "-"}
                        </span>
                      </td>
                      <td className="border border-gray-400 px-2 py-2 text-center">{s(row.note) || "-"}</td>
      ...
      <table className="w-full border-collapse border border-gray-400 text-sm">
        <tbody>
          <tr>
            <td className="border border-gray-400 px-3 py-2 bg-gray-50 font-medium w-1/4">이탈 발생</td>
            <td className="border border-gray-400 px-3 py-2">
              {(fr.rows || []).some((r: any) => r.isDeviation || r.is_deviation) ? "있음" : "없음"}
            </td>
          </tr>
          <tr>
            <td className="border border-gray-400 px-3 py-2 bg-gray-50 font-medium">시정조치</td>
            <td className="border border-gray-400 px-3 py-2">
              {(fr.rows || []).map((r: any) => r.correctiveAction || r.corrective_action).filter(Boolean).join("; ") || "해당없음"}
            </td>
          </tr>
          <tr>
            <td className="border border-gray-400 px-3 py-2 bg-gray-50 font-medium">특이사항</td>
            <td className="border border-gray-400 px-3 py-2">
              {(fr.rows || []).map((r: any) => r.specialNote || r.special_note || r.deviationNote || r.deviation_note).filter(Boolean).join("; ") || " "}
            </td>
          </tr>
        </tbody>
      </table>
...
export default function PrintPreviewPage() {
  ...
  documents.forEach((doc) => {
    ...
    if (doc.formType === "daily_log") {
      ...
      // CCP 기록지(배치): ccpFormRecords 배열을 공정별로 1페이지씩
      ...
      if (ccpRecords.length === 0) {
        ...
        ccpRecords.forEach((fr: any, idx: number) => {
          allPages.push({
            doc: { ...doc, ...safeDocDates, authorName, reviewerName, approverName },
            pageContent: renderCcpFormRecord(fr, doc),
            pageTitle: `CCP 기록지 - ${fr.ccpType || fr.ccp_type || ""} (${fr.processGroupName || fr.process_group_name || ""})`,
            pageIndex: idx,
            totalPages: ccpRecords.length,
          });
        });
      }
