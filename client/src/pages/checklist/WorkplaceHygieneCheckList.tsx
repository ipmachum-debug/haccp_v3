...
export default function WorkplaceHygieneCheckList() {
  ...
  const handleApprovalRequest = (record: any, e: React.MouseEvent) => {
    ...
    if (window.confirm("승인을 요청하시겠습니까?")) {
      approvalMutation.mutate({
        ...
        title: record.title || "workplace_hygiene_check",
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl">작업장 위생 점검</CardTitle>
                  <CardDescription className="mt-1">작성된 기록을 관리합니다</CardDescription>
                </div>
              </div>
              <Button onClick={() => navigate("/workplace-hygiene-check/new")} className="gap-1">
                  ...
                  <TableBody>
                    {filteredRecords.length > 0 ? (
                      filteredRecords.map((record: any, index: number) => (
                        <TableRow key={record.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => navigate(`/workplace-hygiene-check/${record.id}`)}>
                          ...
                          <TableCell className="text-center" onClick={(e) => e.stopPropagation()}>
                            <div className="flex flex-wrap gap-1 justify-center">
                              ...
                              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => navigate(`/workplace-hygiene-check/${record.id}`)}><Edit className="h-3 w-3 mr-1" />수정</Button>
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="text-center py-12 text-muted-foreground">
                          ...
                          등록된 기록이 없습니다.
                          <br />
                          <Button variant="link" className="mt-2" onClick={() => navigate("/workplace-hygiene-check/new")}>
                            새로운 기록 작성하기
                          </Button>
                        </TableCell>
                      </TableRow>
