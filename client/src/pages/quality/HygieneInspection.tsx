...
export default function HygieneInspection() {
  ...
  // 검사 기록 목록 조회
  const { data: records = [], isLoading } = trpc.inspection.hygiene.list.useQuery({});

  // 검사 기록 상세 조회
  const { data: selectedRecord } = trpc.inspection.hygiene.getById.useQuery(
    { id: selectedRecordId! },
    { enabled: !!selectedRecordId }
  );

  // 검사 기록 생성
  const createMutation = trpc.inspection.hygiene.create.useMutation({
    onSuccess: () => {
      toast.success("위생 점검 기록이 생성되었습니다.");
  ...
  // 검사 상태 변경
  const updateStatusMutation = trpc.inspection.hygiene.updateStatus.useMutation({
    onSuccess: () => {
      toast.success("점검 상태가 변경되었습니다.");
      utils.inspection.hygiene.list.invalidate();
  ...
  const handleCreateInspection = () => {
    if (!formData.inspectionArea || !formData.inspectorName) {
  ...
  const handleStatusChange = (id: number, status: "pending" | "completed" | "action_required", result?: "good" | "poor" | "fair") => {
    updateStatusMutation.mutate({ id, status, result });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">위생 점검</h1>
          <p className="text-muted-foreground mt-1">
            생산 시설의 위생 상태를 점검하고 기록합니다.
          </p>
        </div>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>위생 점검 기록 생성</DialogTitle>
              <DialogDescription>
                생산 시설의 위생 상태를 점검하고 기록합니다.
              </DialogDescription>
            </DialogHeader>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>점검 날짜</TableHead>
                  <TableHead>점검 구역</TableHead>
                        ...
                        {record.status === "pending" && (
                          <>
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleStatusChange(record.id, "completed", "good")
                              }
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleStatusChange(record.id, "action_required", "poor")
                              }
                            >
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>위생 점검 상세</DialogTitle>
          </DialogHeader>
          {selectedRecord && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm text-muted-foreground">점검 구역</Label>
                  <p className="font-medium">{selectedRecord.inspectionArea}</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground">점검 날짜</Label>
                ...
                <div>
                  <Label className="text-sm text-muted-foreground">점검자</Label>
                  <p className="font-medium">{selectedRecord.inspectorName || "-"}</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground">판정</Label>
                  <p className="font-medium">
                    {selectedRecord.result === "good"
                      ? "양호"
                      : selectedRecord.result === "poor"
                      ? "불량"
                      : selectedRecord.result === "fair"
                      ? "보통"
                      : "-"}
                  </p>
                </div>
              ...
              <div>
                <Label className="text-sm text-muted-foreground">점검 항목</Label>
