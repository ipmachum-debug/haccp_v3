...
export default function MaterialInspection() {
  ...
  // 검사 기록 목록 조회
  const { data: records = [], isLoading } = trpc.inspection.material.list.useQuery({});

  // 검사 기록 상세 조회
  const { data: selectedRecord } = trpc.inspection.material.getById.useQuery(
    { id: selectedRecordId! },
    { enabled: !!selectedRecordId }
  );

  // 검사 기록 생성
  const createMutation = trpc.inspection.material.create.useMutation({
    onSuccess: () => {
      toast.success("원재료 검사 기록이 생성되었습니다.");
  ...
  const handleCreateInspection = () => {
    if (!formData.materialCode || !formData.materialName || !formData.lotNumber) {
  ...
  const handleStatusChange = (id: number, status: "pending" | "completed" | "rejected", result?: "pass" | "fail" | "conditional") => {
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="원재료명, LOT 번호, 코드로 검색..."
                  value={searchTerm}
                        ...
                        {record.status === "pending" && (
                          <>
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleStatusChange(record.id, "completed", "pass" as "pass" | "fail" | "conditional")
                              }
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleStatusChange(record.id, "rejected", "fail" as "pass" | "fail" | "conditional")
                              }
                            >
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedRecord && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm text-muted-foreground">원재료명</Label>
                  <p className="font-medium">{selectedRecord.materialName}</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground">{L("material")} 코드</Label>
                  <p className="font-medium">{selectedRecord.materialCode}</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground">LOT 번호</Label>
                  <p className="font-medium">{selectedRecord.lotNumber}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedRecord.supplierName || "-"}</p>
                </div>
              </div>
