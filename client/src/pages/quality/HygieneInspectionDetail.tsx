...
export default function HygieneInspectionDetail() {
  const params = useParams<{ id: string }>();
  const [, setLocation] = useLocation();
  const inspectionId = params.id ? parseInt(params.id, 10) : 0;

  const { data: inspection, isLoading, refetch } = trpc.inspection.hygiene.getById.useQuery({ id: inspectionId });
  ...
  const handleStatusChange = (newStatus: "pending" | "completed" | "action_required") => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => setLocation("/dashboard/inspections/hygiene")}
          >
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold">위생 검사 상세</h1>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm font-medium text-muted-foreground">검사 구역</p>
              <p className="text-lg font-semibold">{inspection.inspectionArea}</p>
            </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex gap-2 flex-wrap">
            ...
            <Button
              variant={inspection.status === "action_required" ? "default" : "outline"}
              onClick={() => handleStatusChange("action_required")}
              disabled={updateStatusMutation.isPending || inspection.status === "action_required"}
            >
              조치 필요
            </Button>
          </div>
