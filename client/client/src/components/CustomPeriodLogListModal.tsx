...
interface CustomPeriodLogListModalProps {
  open: boolean;
  onClose: () => void;
  tenantId: number;
}

export function CustomPeriodLogListModal({ open, onClose, tenantId }: CustomPeriodLogListModalProps) {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(`/api/customPeriodLog/get?${params}`);
      if (response.ok) {
  ...
  const handleApprove = async (id: number) => {
    try {
      const response = await fetch(`/api/customPeriodLog/approve/${id}`, {
        method: "POST",
      ...
      if (response.ok) {
        toast({ title: "승인 완료", description: "특정기간일지가 승인되었습니다." });
        fetchLogs();
      }
    } catch (error) {
  ...
  const handleRequestApproval = async (id: number) => {
    try {
      const response = await fetch(`/api/customPeriodLog/requestApproval/${id}`, {
        method: "POST",
      });
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/customPeriodLog/reject/${id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        toast({ title: "반려 완료", description: "특정기간일지가 반려되었습니다." });
        fetchLogs();
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/customPeriodLog/delete/${id}`, {
        method: "DELETE",
      });

      if (response.ok) {
        toast({ title: "삭제 완료", description: "특정기간일지가 삭제되었습니다." });
        fetchLogs();
      }
    } catch (error) {
