...
function SubscriptionInfo({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  // 색상 결정 - 사이드바에 어울리는 부드러운 톤
  let textColor = "text-sidebar-foreground/70";
  let iconColor = "text-sidebar-foreground/50";
  ...
  return (
    <div className="mb-2 rounded-lg border border-sidebar-border p-2.5">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-sidebar-accent text-sidebar-foreground/70 font-medium">
          {packageLabel}
        </span>
      </div>
...
function DashboardLayoutContent({
  ...
  return (
    <>
              ...
              <div className="px-3 mb-2">
                <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "work" | "finance" | "haccp")} className="w-full">
                  <TabsList className="grid w-full grid-cols-3 text-[11px] h-8 bg-sidebar-accent/60">
                    <TabsTrigger value="work" className="text-[11px] h-6 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                      WORK
                    </TabsTrigger>
                    <TabsTrigger value="finance" className="text-[11px] h-6 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">회계</TabsTrigger>
                    <TabsTrigger value="haccp" className="text-[11px] h-6 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">HACCP</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
            )}
                      ...
                      <SidebarMenuItem>
                        <div className="flex items-center gap-1 w-full">
                          <SidebarMenuButton
                            ...
                            tooltip={item.label}
                            className={`h-8 font-normal flex-1 text-[13px] ${isActive ? "bg-sidebar-accent text-sidebar-accent-foreground font-medium" : "text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent/50"} ${(item as any).highlight ? "border border-primary/20 rounded-lg font-medium" : ""}`}
                          >
                            <item.icon
      ...
      <SidebarInset>
        {isMobile && (
          <div className="flex border-b border-border/60 h-12 items-center justify-between bg-background/80 backdrop-blur-xl px-3 sticky top-0 z-40">
            <div className="flex items-center gap-2">
              <SidebarTrigger className="h-9 w-9 p-0 flex items-center justify-center hover:bg-accent rounded-md transition-colors">
        ...
        {!isMobile && (
          <div className="flex border-b border-border/60 h-12 items-center justify-between bg-background/80 backdrop-blur-xl px-5 sticky top-0 z-40">
            <div className="flex items-center gap-2">
        ...
        <main className="flex-1 px-4 py-5 md:px-6 md:py-5">{children}</main>
      </SidebarInset>
    </>
  );
}
