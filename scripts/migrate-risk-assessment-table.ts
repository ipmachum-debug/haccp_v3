/**
 * 마이그레이션: h_risk_assessments 테이블 — Risk Assessment (Phase Y-6)
 *
...
async function migrate() {
  ...
  console.log("=== 마이그레이션 시작: h_risk_assessments (Risk / Phase Y-6) ===\n");

  const [tRows]: any = await conn.execute(
    `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'h_risk_assessments'`,
  );
  const exists = (tRows as any[]).length > 0;

  if (exists) {
    console.log("✅ h_risk_assessments 테이블 이미 존재 — 스킵");
  } else {
    console.log("→ h_risk_assessments 테이블 생성 중...");
    await conn.execute(`
      CREATE TABLE h_risk_assessments (
    ...
    `);
    console.log("✅ h_risk_assessments 테이블 생성 완료");
  }

  const indexes: Array<{ name: string; columns: string; unique?: boolean }> = [
    { name: "uniq_risk_assessment_tenant_code", columns: "tenant_id, code", unique: true },
    { name: "idx_risk_assessment_tenant_industry_status", columns: "tenant_id, industry, status" },
    { name: "idx_risk_assessment_tenant_category_status", columns: "tenant_id, category, status" },
    { name: "idx_risk_assessment_tenant_residual_score", columns: "tenant_id, residual_score" },
  ];

  for (const idx of indexes) {
    const [existsRows]: any = await conn.execute(
      `SELECT INDEX_NAME FROM INFORMATION_SCHEMA.STATISTICS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'h_risk_assessments' AND INDEX_NAME = ?`,
      [idx.name],
    );
    ...
    await conn.execute(
      `CREATE ${uniqueKw}INDEX ${idx.name} ON h_risk_assessments (${idx.columns})`,
    );
